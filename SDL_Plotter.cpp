...
void SDL_Plotter::plotNumber(int num, int x, int y) {
    for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 5; j++) {
            if (digits[num % 10][i][j] == '1') {
                plotPixel(x + j + 9, y + i - 4, 0, 0, 0);
            }
        }
    }
    num /= 10;
    ...
    for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 5; j++) {
            if (digits[num % 10][i][j] == '1') {
                plotPixel(x + j - 12, y + i - 4, 0, 0, 0);
            }
        }
    }
}

void SDL_Plotter::plotFilledCircle(int r, point p, color c) {
    for (int x = -r; x <= r; x++) {
        for (int y = -r; y <= r; y++) {
            if (sqrt(pow(x, 2) + pow(y, 2)) <= r) {
                plotPixel(x + p.x, y + p.y, c);
            }
        }
    }
}

void SDL_Plotter::plotLine(point p1, point p2, color c) {
    int dx = p2.x - p1.x;
    int dy = p2.y - p1.y;
