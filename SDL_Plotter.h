...
private:
    ...
    void readLetters() {
        ifstream inFile("digits.txt");
        string trash;
        getline(inFile, trash);
        for (int i = 0; i < 10; i++) {
            for (int row = 0; row < 9; row++) {
                for (int col = 0; col < 5; col++) {
                    digits[i][row][col] = inFile.get();
                }
                inFile.ignore();
            }
            getline(inFile, trash);
...
public:
    ...
    void drawCircle(int r, point p, color c);

    void plotFilledCircle(int r, point p, color c);

    void plotLine(point p1, point p2, color c);

    void displayNode(int val, int x, int y, color c = {0, 0, 0});
};
